...
df = pd.read_csv('peaking.csv', delimiter=',', parse_dates=[0], header=None, names=['x', 'd', 'dd', 'ddd'])
...
plt.plot(df['x'], df['ddd'], label='Fiber Download')
...
smooth3 = savgol_filter(df.ddd.values, window_length = 351, polyorder = 5)
...
plt.plot(df['x'], smooth3, label='Smoothed Fiber Download')
